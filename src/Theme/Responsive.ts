 ...
 */
const convertFontScale = (fontSize: string): number => {
  // Parse fontSize convert according to screen width.
  const baseSize: Number = Platform.select({ios: 414, android: 420});
  return Number(fontSize) * (screenWidth / baseSize);
};
 ...
 */
const listenOrientationChange = (that: object) => {
